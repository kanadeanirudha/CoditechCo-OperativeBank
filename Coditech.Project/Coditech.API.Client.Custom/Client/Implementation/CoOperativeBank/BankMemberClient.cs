...
namespace Coditech.API.Client
{
    public class BankMemberClient : BaseClient, IBankMemberClient
    {
        ...
        public virtual BankMemberResponse GetMemberOtherDetail(int bankMemberId)
        {
            return Task.Run(async () => await GetMemberOtherDetailAsync(bankMemberId, CancellationToken.None)).GetAwaiter().GetResult();
        }

        public virtual async Task<BankMemberResponse> GetMemberOtherDetailAsync(int bankMemberId, CancellationToken cancellationToken)
        {
            if (bankMemberId <= 0)
                throw new ArgumentNullException("bankMemberId");

            string endpoint = bankMemberEndpoint.GetMemberOtherDetailAsync(bankMemberId);
            HttpResponseMessage response = null;
            var disposeResponse = true;
            try
            {
        ...
        public virtual BankMemberResponse UpdateMemberOtherDetail(BankMemberModel body)
        {
            return Task.Run(async () => await UpdateMemberOtherDetailAsync(body, CancellationToken.None)).GetAwaiter().GetResult();
        }

        public virtual async Task<BankMemberResponse> UpdateMemberOtherDetailAsync(BankMemberModel body, CancellationToken cancellationToken)
        {
            string endpoint = bankMemberEndpoint.UpdateMemberOtherDetailAsync();
            HttpResponseMessage response = null;
            var disposeResponse = true;
            try
            {
